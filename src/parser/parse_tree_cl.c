...
#if defined (PT_TRACE_DEV)
/* 
 *
 * PT_TRACE_DEV
 * 
 * This code provides debug utilities for tracing the parse tree nodes and stack traces during the execution of dblink
 * -> prefetch -> pt_compile -> prepare.
 * The utilities are conditionally included based on the macro `PT_TRACE_DEV`, which allows developers to enable or
 * disable these features for development and debugging purposes.
 *
 * To enable these debug utilities, ensure that the macro `PT_TRACE_DEV` is
 * defined during the compilation. This can be achieved by
 *
 * $ cmake -DENABLE_PT_TRACE_DEV=ON
 *
 * or, adding a cacheVariable in CMakePresets.json (recommended)
 *
 * # CMakePresets.json
      ...
      "cacheVariables": {
      ...
        "ENABLE_PT_TRACE_DEV":"ON"
      }
      ...
 *
 * Usage:
 * ------
 * To use these utilities in your parser code, you need to wrap your parse tree walk function calls with the
 * `PT_APPLY_WALK` macro. For example:
 *
 * void someFunction(Parser *parser, PT_NODE *node) {
 *     PT_APPLY_WALK(parser, node, someArgument);
 * }
 *
 * This will ensure that if the debug macro is enabled, additional debug information is printed to help trace and
 * diagnose issues with the parse tree.
 *
 * Notes:
 * ------
 * - The `PT_TRACE_DEV` macro should be defined only in development or debug builds to avoid the performance overhead
 *   in production builds.
 * - The `execinfo.h` library is specific to Unix-like systems and may not be available on all platforms.
 */
...
std::string ptNodeValueToString (PT_NODE * node)
{
...
std::ofstream logfile ("pt_trace.log");
...
logStackTrace ()
{
  ...
  if (addrlen == 0)
    {
      logfile << "No stack trace available\n";
      return;
    }
...
logNode (PT_NODE * node)
{
  ...
  logfile << std::left << std::setw (indentWidth)
    << indent + ptNodeTypeToString (node->node_type) +
    ptNodeValueToString (node) << " with address of " << node << "\n\n";
}

void
logCaller (const char *func, const char *ptr_name)
{
  logfile << std::string (pt_walk_recursion_level * 4, ' ') << "At " << func << "(), `" << ptr_name << "` is\n";
}
