...
class ServerEnv : public ::testing::Environment
{
  ...
  private:
    void StartServer()
    {
      printf ("Starting CUBRID server for tests...\n");
      er_init ("./test_cubrid.log",ER_NEVER_EXIT);

      // hacky way to detour pl_server_init(), not needed for tests
    ...
    void StopServer()
    {
      printf ("Stopping CUBRID server for tests...\n");
